...
`)
	...
	for _, serviceName := range []string{"ui", "db"} {
		path := filepath.Join(templatesDir, "configmap-"+serviceName+"-environment.yaml")
		if _, err := os.Stat(path); !os.IsNotExist(err) {
			t.Fatalf("expected no environment ConfigMap for %s, stat err = %v", serviceName, err)
		}
	}
