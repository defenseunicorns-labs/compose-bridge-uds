...
// that are intentionally omitted from the package. Invalid references on an
// excluded service are ignored here because excluded services otherwise bypass
...
// markBoundarySecretsExternal keeps local Compose secret files useful for
// excluded development dependencies while making the packaged workload consume
// an existing Kubernetes Secret supplied by its deployment environment.
