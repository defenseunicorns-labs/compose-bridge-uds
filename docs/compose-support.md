...
| Bind mounts                                                      | Rejected because host paths are unavailable to the transformation and have no portable Kubernetes equivalent. Use named `volumes:`, `configs:`, or `secrets:` for data that should be rendered into the chart.                                                                  |
...
- **Bind mounts:** Bind mounts are treated as local-development-only inputs and stop conversion with an actionable compatibility error. Use a named volume when the data should become a PVC, or a config or secret when the mounted content should be materialized in Kubernetes.
