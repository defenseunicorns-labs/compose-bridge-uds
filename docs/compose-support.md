...
When `image:` is omitted, Compose assigns a default image name based on the project and service, such as `hello-world-server` for `examples/full`. Declare `image:` before building to make the generated Helm chart and `zarf.yaml` use a stable or registry-backed reference.
