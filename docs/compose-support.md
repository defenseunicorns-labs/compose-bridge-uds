...
| `networks:`                                                      | Service membership is preserved with Pod labels and selector-scoped UDS ingress and egress rules when services use different network sets. External networks warn because external peers cannot be inferred; aliases, addresses, and driver options remain unsupported.            |
